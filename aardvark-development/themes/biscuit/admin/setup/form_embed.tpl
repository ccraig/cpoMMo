...
<h2><img src="{$url.theme.shared}images/icons/embed.png" alt="embed" class="navimage left" />{t}Embedded Subscription Forms{/t}</h2>

{t}Subscription forms can easily be added to your website using a line of code. You can use the PHP include listed below, or embed the actual HTML. Remember, you can also direct subscribers to the {/t}<a href="{$url.base}user/subscribe.php">{t}Default Subscribe Form{/t}</a>.

<br class="clear"/>

<h3>{t}Mini Subscription Form{/t}</h3>
{t}This prints a form which prompts for a user's email address. If the inputted email exists as a registered subscriber, it redirects to the subscriber update page. If not, it redirects to the the default subscription form.{/t}
<p>PHP CODE TO USE: <span style="color: green;">include('{$config.app.path}embed.miniform.php');</span></p>
...
{t}This prints the entire subscription form.{/t}
<p>PHP CODE TO USE: <span style="color: green;">include('{$config.app.path}embed.form.php');</span></p>
