...
class PHPMailer {
	...
	// bMAIL VARS
	/**
	 *  Holds the size (in bytes of the last sent message)
	 *  NOTE: not accurate with multibyte / mb_string character encodings
	 *  @var string
	 */
	var $LastMessageSize = 0;
	
	var $SaveMessageSize = FALSE;
	...
	function Send() {
		...
		switch ($this->Mailer) {
			...
			case "smtp" :
				...
				break;
			default :
				$this->SetError($this->Mailer . $this->Lang("mailer_not_supported: " . $this->Mailer));
				$result = false;
				break;
		}
		
		// bMAIL CALL
		// Get the size (in bytes) of this message and update placeholder
		if ($this->SaveMessageSize && $result)
			$this->LastMessageSize = strlen($header) + strlen($body);
